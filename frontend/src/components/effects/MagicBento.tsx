...
export const MagicBento: React.FC<MagicBentoProps> = ({
  ...
  const cards = items || [
    { title: 'Live Classroom', description: 'Real-time video & screen share', label: 'Agora RTC' },
    { title: 'AI Voice Tutor', description: 'Ask doubts privately with zero audio leak', label: 'Multilingual' },
    { title: 'Course Grounded', description: 'Cited answers directly from class PDFs', label: 'Accurate RAG' },
    { title: 'Zero Friction', description: 'Works on any device without installs', label: 'Web-Native' },
  ];
      ...
      {cards.map((card, index) => {
        ...
        return (
          <ParticleCard
            ...
            style={{
              backgroundColor: card.color || '#0D1426',
              // @ts-ignore
          ...
          >
            <div className="magic-bento-card__header flex items-center justify-between">
              {card.label && <span className="text-[11px] font-bold text-cyan-400 bg-cyan-950/60 border border-cyan-800/40 px-2.5 py-0.5 rounded-full">{card.label}</span>}
              {card.icon}
            </div>
